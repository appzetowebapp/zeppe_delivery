...
class _WebViewScreenState extends State<WebViewScreen>
  ...
  static const platform =
      MethodChannel('com.zeppe.deliveryapp1/geolocation');
  InAppWebViewController? _webViewController;
  bool _isLoading = true;
  double _loadingProgress = 0.0;
        ...
        (function() {
          ...
          window.fetch = async function(url, options) {
            ...
            var isLogin = urlString.includes('/auth/login') || 
                          urlString.includes('/users/login') ||
                          urlString.includes('/auth/signup-verify') ||
                          urlString.includes('/delivery/verify-otp');
          ...
          XMLHttpRequest.prototype.send = function(data) {
            ...
            if (url && (url.includes('/auth/login') || 
                        url.includes('/users/login') ||
                        url.includes('/auth/signup-verify') ||
                        url.includes('/delivery/verify-otp'))) {
               this.addEventListener('load', function() {
                  try {
                    var responseBody = self.responseText;
                    // Try parsing JSON
    ...
    if (args.isNotEmpty) {
      try {
        ...
        if (targetMap != null) {
          ...
          debugPrint('⚠️ Neither "result" nor "body" fields found in the response.');
        }
      } catch (e) {
        debugPrint('❌ Error parsing login/signup response: $e');
      }
